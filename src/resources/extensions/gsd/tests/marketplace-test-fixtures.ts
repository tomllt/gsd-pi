...
export function getMarketplaceFixtures(testFileDir: string): { available: boolean; skipReason?: string; fixtures?: MarketplaceFixtureSet } {
  const gsdPiRoot = resolve(testFileDir, '../../../../..');
  const localClaudeSkillsPath = resolve(gsdPiRoot, '../claude_skills');
  const localClaudePluginsOfficialPath = resolve(gsdPiRoot, '../claude-plugins-official');
